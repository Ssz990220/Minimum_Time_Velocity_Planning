...
%     y_z = zeros(size(ds, 1)-1,1);
    ...
    for i = 2:size(s,2)-1
        ...
        [~, f_order] = sort(f_inv_slope);
        x = min(phi.^2/qds(i,:).^2);
        while y_bar < z_bar
            m = size(b_slope,1);
            j = size(f_inv_slope,1);
            ...
            while ((j>1) && (f_inv_slope(f_order(j))*x+f_inv_intercept(f_order(j)))<f_inv_slope(f_order(j-1))*x+f_inv_intercept(f_order(j-1)))
                j = j-1;
            end
            z_bar = f_inv_slope(f_order(j))*x+f_inv_intercept(f_order(j));
            x_pre = x;
            x = (1/f_inv_slope(f_order(j)))*b_intercept(b_order(m)) + (-f_inv_intercept(f_order(j))/f_inv_slope(f_order(j)))/(1-1/f_inv_slope(f_order(j))*b_slope(b_order(m)));
        end
